...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.declare_parameter('weights_path', '.yolov5.pt')
        ...
        path_model = self.get_parameter('weights_path').value
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=path_model, force_reload=True)
    ...
    def getDetectionArray(self, df):
        ...
        for row in df.itertuples():
            ...
            center = Pose2D()
            center.position.x = (int(row.xmin) + int(row.xmax)) / 2
            center.position.y = (int(row.ymin) + int(row.ymax)) / 2
    ...
    def listener_callback(self, data):
        ...
        if self.get_parameter('pub_image').value:
            processed_image = self.br.cv2_to_imgmsg(results.plot())
